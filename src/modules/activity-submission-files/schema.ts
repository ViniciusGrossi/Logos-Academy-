...
const GithubUrlSchema = HttpsUrlSchema.superRefine((value, context) => {
  try {
    if (new URL(value).hostname !== "github.com")
      context.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Use um repositório github.com.",
      });
  } catch {
    /* HttpsUrlSchema reports malformed URLs. */
  }
});

export const AssignmentPathSchema = z
  .object({ assignmentId: UuidSchema })
  .strict();
export const ActivityDetailQuerySchema = AssignmentPathSchema.extend({
...
export const ProfilePatchSchema = z
  ...
  .refine(
    (input) =>
      input.displayName !== undefined || input.githubUsername !== undefined,
...
export const DraftInputSchema = z
  .object({
    ...
    items: z
      ...
      .refine(
        (items) =>
          new Set(items.map((item) => item.requirementId)).size ===
          items.length,
        "Requisitos não podem se repetir.",
      ),
  })
  .strict();
export const SubmitInputSchema = z
  .object({ assignmentId: UuidSchema, expectedDraftId: UuidSchema })
  .strict();
