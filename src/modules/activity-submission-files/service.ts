...
export class ActivitySubmissionFilesService {
  ...
  async profile(actor: StudentActor, requestId: string): Promise<MeProfile> {
    return {
      ...(await this.repository.getProfile(actor, requestId)),
      email: actor.email,
      role: actor.role,
    };
  }
  async patchProfile(
    ...
    return {
      ...(await this.repository.updateProfile(
        actor,
        parse(ProfilePatchSchema, input, requestId),
        requestId,
      )),
      email: actor.email,
      role: actor.role,
  ...
  async detail(
    ...
    const parsed = parse(
      ActivityDetailQuerySchema,
      { assignmentId, ...(cursor === undefined ? {} : { cursor }) },
      requestId,
    );
  ...
  async submit(
    ...
    const draft =
      detail.latestSubmission?.id === parsed.expectedDraftId &&
    ...
    const missing = detail.requirements.filter(
      (requirement) =>
        requirement.required && !presentRequirementIds.has(requirement.id),
    );
    if (draft && missing.length > 0) {
      throw new AppError(
        "VALIDATION_ERROR",
        "Preencha todos os entregáveis obrigatórios.",
        requestId,
        Object.fromEntries(
          missing.map((requirement) => [
            `items.${requirement.label}`,
            ["Entregável obrigatório ausente."],
          ]),
        ),
      );
    }
    if (
      detail.requirements.some(
        (requirement) =>
          requirement.required && requirement.kind === "github_repository",
      ) &&
      !(await this.repository.getProfile(actor, requestId)).githubUsername
    ) {
      throw new AppError(
        "VALIDATION_ERROR",
        "Configure seu GitHub antes de enviar esta atividade.",
        requestId,
        { githubUsername: ["GitHub obrigatório para esta atividade."] },
      );
    }
    return this.repository.submit(
      actor,
      parsed.assignmentId,
      parsed.expectedDraftId,
      requestId,
    );
  }
  async createUpload(
    ...
    const created = await this.repository.createUpload(
      actor,
      parse(UploadInputSchema, input, requestId),
      requestId,
    );
    const url = await this.repository.createSignedUploadUrl(
      created.storagePath,
      requestId,
    );
    return {
      file: created.file,
      signedUploadUrl: url.signedUrl,
  ...
  async finalize(
    ...
    return this.repository.finalize(
      actor,
      parse(FilePathSchema, { fileId }, requestId).fileId,
      requestId,
    );
  }
  async download(
    ...
    const file = await this.repository.fileMetadata(
      actor,
      parse(FilePathSchema, { fileId }, requestId).fileId,
      requestId,
    );
...
function parse<T>(
  ...
  if (!parsed.success)
    throw new AppError("VALIDATION_ERROR", "Dados inválidos.", requestId);
  return parsed.data;
}
