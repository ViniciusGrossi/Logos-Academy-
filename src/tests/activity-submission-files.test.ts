...
const pedagogy = { context: null, expectedResult: null, steps: [], planB: null, reflectionPrompt: null, portfolioEvidence: null, toolHint: null } as const;
...
describe("activity-submission-files", () => {
  ...
  it("GWT-05 bloqueia envio quando a atividade exige GitHub sem perfil configurado", async () => {
    ...
    repository.detail = async () => ({ id: fileId, assignmentId, lessonPosition: 1, title: "Atividade", objective: "Objetivo", instructions: "Instruções", continuityGuidance: "Continue", estimatedMinutes: 10, dueAt: null, status: "draft" as const, isOverdue: false, canEdit: true, readOnlyReason: null, supplementalInstructions: null, ...pedagogy, project: null, concepts: [], requirements: [{ id: fileId, kind: "github_repository" as const, label: "Repositório", required: true, position: 1 }], criteria: [], latestReview: null, latestSubmission: null, submissionHistory: { items: [], nextCursor: null } });
  ...
  it("Explorer v2 permite enviar sem GitHub quando o requisito é opcional", async () => {
    const repository = new Store();
    repository.detail = async () => ({ id: fileId, assignmentId, lessonPosition: 15, title: "Build Day", objective: "Objetivo", instructions: "Instruções", continuityGuidance: "Continue", estimatedMinutes: 45, dueAt: null, status: "draft" as const, isOverdue: false, supplementalInstructions: null, ...pedagogy, project: null, canEdit: true, readOnlyReason: null, concepts: [], requirements: [{ id: fileId, kind: "github_repository" as const, label: "Repositório final", required: false, position: 1 }], criteria: [], latestReview: null, latestSubmission: { id: fileId, assignmentId, version: 1, isDraft: true, isLate: false, submittedAt: null, items: [], review: null, reviews: [] }, submissionHistory: { items: [], nextCursor: null } });
    repository.submit = async () => ({ id: fileId, assignmentId, version: 1, isDraft: false, isLate: false, submittedAt: new Date().toISOString(), items: [], review: null, reviews: [] });
    await expect(new ActivitySubmissionFilesService(repository).submit(actor, { assignmentId, expectedDraftId: fileId }, "request")).resolves.toMatchObject({ isDraft: false });
  });

  it("lista nominalmente todos os entregáveis obrigatórios ausentes", async () => {
    const repository = new Store();
    repository.detail = async () => ({ id: fileId, assignmentId, lessonPosition: 1, title: "Atividade", objective: "Objetivo", instructions: "Instruções", continuityGuidance: "Continue", estimatedMinutes: 10, dueAt: null, status: "draft" as const, isOverdue: false, supplementalInstructions: null, ...pedagogy, project: null, canEdit: true, readOnlyReason: null, concepts: [], requirements: [{ id: fileId, kind: "text" as const, label: "Registro dos testes", required: true, position: 1 }], criteria: [], latestReview: null, latestSubmission: { id: fileId, assignmentId, version: 1, isDraft: true, isLate: false, submittedAt: null, items: [], review: null, reviews: [] }, submissionHistory: { items: [], nextCursor: null } });
    await expect(new ActivitySubmissionFilesService(repository).submit(actor, { assignmentId, expectedDraftId: fileId }, "request")).rejects.toMatchObject({ code: "VALIDATION_ERROR", fieldErrors: { "items.Registro dos testes": ["Entregável obrigatório ausente."] } });
  });
