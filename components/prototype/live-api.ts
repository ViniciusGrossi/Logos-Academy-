...
export async function apiMutation<T>(url: string, method: "POST" | "PUT" | "PATCH" | "DELETE", body: unknown, headers?: Record<string, string>): Promise<T> {
  if (isDemoMode()) return demoApi<T>(url, method, isJsonObject(body) ? body : undefined);
  const response = await fetch(url, { method, credentials: "same-origin", headers: { "content-type": "application/json", ...headers }, body: JSON.stringify(body) });
  const result: unknown = await response.json();
