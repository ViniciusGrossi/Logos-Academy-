  ...
  return <div className={styles.page}>
    ...
    <div className={styles.twoPane}>
      <aside className={styles.index} aria-label="Índice de conceitos">
        ...
        {list.loading ? <StateScene state="loading" title="Organizando conceitos" description="Preparando o índice da sua turma." /> : list.error ? <StateScene state="error" title="Não foi possível abrir o glossário" description={list.error.message} action={<button onClick={() => void list.reload()}>Tentar novamente</button>} /> : items.length === 0 ? <StateScene state="empty" title="Nenhum conceito encontrado" description={search ? "Tente uma palavra diferente." : "Os conceitos aparecem conforme as aulas presenciais avançam."} /> : <div className={styles.conceptList}>{items.map((concept,index) => <button key={concept.id} type="button" className={styles.concept} data-active={selectedId === concept.id} onClick={() => setSelectedId(concept.id)}><span className={styles.conceptIndex}>{String(index+1).padStart(2,"0")}</span><span><strong>{concept.title}</strong><small>{concept.summary}</small></span><ChevronRight aria-hidden="true" /></button>)}</div>}
      </aside>
