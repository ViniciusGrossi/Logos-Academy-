    ...
    <header className={styles.pageHeading}>
      ...
      <div className={styles.headingSignature} aria-hidden="true">
        ...
        <svg viewBox="0 0 520 210" role="presentation">
          ...
          <circle className={styles.signatureScanner} r="5">
    ...
    <div className={styles.profileLayout}>
      <aside className={styles.passport} aria-label="Passaporte do estudante">
        ...
        <div className={styles.passportFacts}>
          ...
          <div><span className={styles.factIcon}><Orbit size={17} /></span><small>Ciclo atual</small><strong>{currentProject ? `${String(currentProject.cyclePosition).padStart(2, "0")} · ${currentProject.title}` : "Em preparação"}</strong></div>
        </div>
        ...
        <section className={styles.cycleVisas} aria-label="Vistos dos ciclos">
          ...
          <ol>
            {projects.map((project) => {
              ...
              return <li key={project.id} data-state={state} title={`Ciclo ${project.cyclePosition}: ${project.title}`}>
                <span>{state === "complete" ? <Check size={17} /> : String(project.cyclePosition).padStart(2, "0")}</span>
                <small>{state === "complete" ? "Concluído" : state === "current" ? "Em curso" : "A seguir"}</small>
              </li>;
            })}
          </ol>
        </section>
        ...
        <div className={styles.passportSeal} key={recordVersion} data-updated={recordVersion > 0} aria-label={recordVersion > 0 ? "Registro atualizado" : "Registro válido"}>
      ...
      <section className={styles.recordDesk}>
        <header className={styles.deskHeader}>
          <div><span className={styles.eyebrow}>Mesa de registro · edição</span><h2>Dados que assinam suas evidências</h2></div>
          <StatusBadge tone={dirty ? "warning" : "success"}>{dirty ? "Alterações não salvas" : "Registro sincronizado"}</StatusBadge>
        </header>

        <form className={styles.profileForm} onSubmit={save}>
          <div className={styles.fieldGroup}>
            <label htmlFor="displayName">Nome de exibição</label>
            <div className={styles.inputShell}><CircleUserRound size={18} /><input id="displayName" value={displayName} onChange={(event) => setDisplayName(event.target.value)} minLength={1} maxLength={120} required /><span>Autoria</span></div>
            <small>Este nome aparece nas atividades, evidências e no portfólio interno.</small>
          </div>

          <div className={styles.fieldGroup}>
            <label htmlFor="profileEmail">E-mail de acesso</label>
            <div className={styles.inputShell} data-readonly><Mail size={18} /><input id="profileEmail" value={profile.data.email} readOnly aria-readonly="true" /><span>Protegido</span></div>
            <small>O endereço de acesso é administrado pela matrícula e não pode ser alterado aqui.</small>
          </div>
          ...
          <section className={styles.githubLink} data-connected={githubConnected}>
            ...
            <div className={styles.githubField}>
              <label htmlFor="githubUsername">github.com/</label>
              <input id="githubUsername" value={githubUsername} onChange={(event) => setGithubUsername(event.target.value)} pattern="[A-Za-z0-9](?:[A-Za-z0-9-]{0,37}[A-Za-z0-9])?" placeholder="seu-usuario" />
              <span><Link2 size={14} /> {githubUsername ? "Identificador informado" : "Aguardando usuário"}</span>
            </div>
          </section>

          <div className={styles.formFooter}>
            <div className={styles.recordState} aria-live="polite">
            ...
            <MagneticAction className={styles.saveMagnetic}><button type="submit" className={styles.saveButton} disabled={pending || !dirty}><Save size={17} /> {pending ? "Atualizando…" : dirty ? "Atualizar passaporte" : "Tudo salvo"}</button></MagneticAction>
          </div>
        </form>
      </section>
    </div>
